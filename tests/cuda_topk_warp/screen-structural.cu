...
int main(int argc,char **argv) {
    ...
    bool structural=argc>1&&!strcmp(argv[1],"--structural");
    bool quick=structural||(argc>1&&!strcmp(argv[1],"--quick"));
    ...
    for(uint32_t top:{512u,2048u}) for(uint32_t rows:batches) for(uint32_t n:widths) {
        ...
        for(int pattern=0;pattern<(structural?1:9);pattern++) {
            ...
            const std::vector<int> order=structural?std::vector<int>{0,1}:std::vector<int>{0,1,1,0};
            for(int mode:order) {
