...
static int check_case(const void *model, uint64_t model_size,
    ...
    for (int run = 0; run < 3 && ok; run++) {
        ...
        /* The reference materializes gate/up before SwiGLU. Fused kernels
         * may leave those scratch tensors untouched; compare their computed
         * weighted activation and outputs against this independent path. */
        ...
        for (int i = 0; i < 5 && ok; i++) {
            if (run != 0 && i < 2) continue; /* optional fused scratch */
